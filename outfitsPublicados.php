...
require_once("biblioteca.php");
 session_start();
  ...
  if($sesion=="")
  {
      echo cabecera($sesion,"escandallos",2);
  }
  else
  {
    echo cabecera($sesion,"escandallos",5);  
 
  }
  $con =conbd();
  echo "<h1  class='col-md-9 ' > <img    src='images/outfitad.png' width='40'/>Outfits Publicados</h1>";

  
         $sql="SELECT * From receta where publico='1'order by Nombre";
               ...
               printf("<td   data-toggle='modal' data-target='#myModal".$fila['idReceta']."' style='cursor: pointer'  class='lista'  >%s</td>
                        <td   data-toggle='modal' data-target='#myModal".$fila['idReceta']."' style='cursor: pointer'  class='lista' >&nbsp; %0.2f€</td>
                         <td  data-toggle='modal' data-target='#myModal".$fila['idReceta']."' style='cursor: pointer'   class='lista' > &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;%s</td>
                          <td   data-toggle='modal' data-target='#myModal".$fila['idReceta']."' style='cursor: pointer'  class='lista' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;%0.2f€</td>
                          <td   data-toggle='modal' data-target='#myModal".$fila['idReceta']."' style='cursor: pointer'  class='lista' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;%s %%&nbsp&nbsp</td>
 ...
 //------------------------------------------------------------------------------------------------------ 
  $sql0="SELECT * From receta where publico='1'order by Nombre";
        ...
        if($num>0)
        {
            echo "<table  class= 'table ' style='font-size:14px'>";
             ...
             echo    " <div  class='col-sm-12 ficha' style='background-color:#DAEFF8;border-radius: 15px;margin-top:14px;'  >
                              <span class='sec' style='font-size:14px;'><b>Descripcíon:</b></span><br/> ".$fila0['descripcion']."
                        </div>
