...
if (isset($_GET['id']))
{
 ...
 if($haydatos)
    {
        ...
        while ($fila=mysqli_fetch_array($res))
        {
            ...
            $sql1= sprintf("UPDATE productosLote set numeroLote='%s', fechaCaducidad='%s',imagen='%s',nombre='%s',marca='%s'
                                                           WHERE idProductoLote='%s'",$_POST["lote".$fila["idListaCompra"]],$_POST["f".$fila["idListaCompra"]],$_POST["nombre".$fila["idListaCompra"]],$_POST["marca".$fila["idListaCompra"]],$nombreImagen,$_POST["idProductoLote".$fila["idProductoLote"]]);
 ...
 //echo "id lote ".$idLote;
       ...
       echo "<form name='frm' method='post' class='form-horizontal' action='prendasTemporada.php?id=".$idLote."'  class='form-group '  enctype='multipart/form-data' >";
        echo "<h1  class='col-sm-12 ' style='margin-bottom:24px'> <img    src='images/lotes.png' width='40'/>Registro de ".$nombreReceta."</h1>Introduzca el numero colección o una foto y la fecha de entrega de cada prenda:<br/><br/>";
         $sql="SELECT * From productoslote where idLote='".$idLote."' ";
         $res=mysqli_query($con, $sql);  
        // echo $sql;
        while ($fila=mysqli_fetch_array($res))
        {
         ...
         $fila1=mysqli_fetch_array($res1);
          echo "<div class='form-group  ' >";
           ...
           if(!$fila["imagen"]=="")
                    echo "<img  width='120' src='imgUsu/".$fila["imagen"]."'>";
             echo "</div>";
                
                echo caja("lote".$fila["idListaCompra"],"",$fila["numeroLote"]);
               
             echo "</div  >";
             echo "<div class='col-sm-4' style='margin-top:14px'>";   
                 echo "<input type='date' name='f".$fila["idListaCompra"]."' value='".$fila['fechaCaducidad']."' class='form-control' id='dp' />";
             echo "</div  >";
         echo "</div  >";
